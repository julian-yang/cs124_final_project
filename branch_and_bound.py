...
def branch_and_bound(input, greedy_solution_count):
    ...
    for genotype1 in genotypes_with_two_resolutions:
        (h1, h2, h3, h4, valid) = determine_h1_h2_h3_h4(genotype1, genotype_solutions, haplotype_resolution)
        if not valid:
            continue
        for genotype2 in genotypes_with_two_resolutions:
            if genotype1 == genotype2:
                continue
            (h5, h6, h7, h8, valid) = determine_h1_h2_h3_h4(genotype2, genotype_solutions, haplotype_resolution)
            if not valid:
                continue
            if (h2 == h6 and h4 == h8) or (h2 == h8 and h4 == h6):
                # delete h3,h4 and h7,h8 (aka the latter 2 pairs of each genotype)
